...
*/
   ...
   while (*p) {
     if ( *p == '-' && (p == infixstr || *(p-1) == '(')  ) {
      ...
      // your code, get the value of the symobol from the hash table, then enqueue the value
       HNODE *found = hashtable_search(ht, symbol);
       if (found) {
           enqueue(&queue, new_node(sign * found->data.value, 0));
           sign = 1;
       } else {
           // symbol not found in hash table
 ...
 int evaluate_infix_symbol(HASHTABLE *ht, char *infixstr) {
     // your code
     // convert infix to postfix using the previously defined function
